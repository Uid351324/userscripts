...
//  	console.log("start isSeries "  + isSeries);
	...
	if(isSeries)
	{
		...
		// {
		// 	populateCovers(rows[i], coverListPeri, isSeries, isAuthor);
		// 	// rows[i].parentNode.className = "hideRowPeri";
		// }
		// var list_table = document.getElementsByClassName("series_rows_tablePeri")[0];
		// list_table.parentNode.insertBefore(coverListPeri, list_table);
	...
	else if(isGroup)
	{
		...
		// for (i = 0; i < rows.length; i++) 
		// {
		// 	populateCovers(rows[i], coverListPeri, isSeries, true);
		// }
	...
	else
	{
		...
		{
			var list_table = document.querySelectorAll('.characterlist > tbody')[0];
			...
			if(list_table)
			{
				...
				coverListPeri.appendChild(coverListPeriTest);
				coverListPeriTest.className = "testPeri";
				rows = list_table.querySelectorAll('tr:not(.rowspan)');
				for (i = 0; i < rows.length; i++) 
				{
...
function populateCovers(element, coverListPeri, isSeries, isAuthor)
{
	...
	if (!id ) {
		console.log("undef: " );
		id = element.querySelectorAll(".name  a")[0].href.split('id=')[1];
		element.id = id;
	}
	if(isSeries)
	{
...
// 		}
	...
	else if(isAuthor)
	{
		...
		text = element.querySelectorAll(".name  a")[0].textContent;
	}
	img.alt = text;
